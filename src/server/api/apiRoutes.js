/*
 * Copyright (C) 2016 DivDE <divde@laposte.net>
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
"use strict";

import loginRoutes from "./login/loginRoutes";
import checkCsrfToken from "./login/checkCsrfToken";
import wrapErrors from "./wrapErrors";
import revisionManager from "./revisionManager";
import validateEnvelope from "@validation/envelope";
import validateDeposit from "@validation/deposit";
//import validateUser from "@validation/user";

export default [
    {
        path: "/*",
        actions: [wrapErrors]
    },
    {
        path: "/api",
        actions: [checkCsrfToken],
        children: [
            loginRoutes,
            revisionManager({
                path: "/envelope",
                collectionName: "envelopes",
                validator: validateEnvelope
            }),
            revisionManager({
                path: "/deposit",
                collectionName: "deposits",
                validator: validateDeposit
            })/*,
            revisionManager({
                path: "/user",
                collectionName: "users",
                validator: validateUser,
                writeRole: "admin",
                readHistoryRole: "admin",
                computeKeys: (user) => ({ email: user.get("email") })
            })*/
        ]
    }
];
